...
-- using (≤-trans; ≤-reflexive; <-transˡ; <-transʳ; ≤-total)

open import IMP
open import MaxProperties
...
... | yes q rewrite q = ⊥-elim (contra r x₁)
... | no  _ = refl
...
reversal₁ (WhileFalse x) v = ⊥-elim (true≢false (trans (sym v) x))
...
reversal₂ (WhileTrue x r r₁) v = ⊥-elim (true≢false (trans (sym x) v))
...
non-interference {l = l} (IfTrue {b = b} x cs) (IfFalse x₁ red) (SecIf w w₁) e r r₁ =
  [ (λ secb≤l →
        let wr = non-interference-bexp b e secb≤l
         in ⊥-elim (true≢false (trans (sym x) (trans wr x₁))))
...
non-interference {l = l} (IfFalse x₁ red) (IfTrue {b = b} x cs) (SecIf w w₁) e r r₁ = 
  [ (λ secb≤l →
        let wr = non-interference-bexp b e secb≤l
         in ⊥-elim (true≢false (trans (sym x) (trans (sym wr) x₁))))
  , (λ l<secb → let oo₁ = confinement cs w
                    oo₂ = confinement red w₁
                 in trans (sym (oo₂ r (<-transʳ r₁ l<secb)))
                   (trans (e r r₁)
...
non-interference {l = l} (WhileFalse {b = b} r) (WhileTrue c ct ct₁) (SecWhile z) e x x₁ =
  [ (λ secb≤l →
        let wr = non-interference-bexp b e secb≤l
         in ⊥-elim (true≢false (trans (sym c) (trans (sym wr) r))))
  , (λ l<secb →
        let simp = confinement ct z
            simp2 = confinement ct₁ (SecWhile (level-cong (sym (m≤n⇒m⊔n≡n l<secb)) z))
         in trans (e x x₁) (trans (simp x (<-transʳ x₁ l<secb)) (simp2 x (missing x₁))))
  ]  (base (sec₆ b) l)
non-interference {l = l} (WhileTrue c ct ct₁) (WhileFalse {b = b} r) (SecWhile z) e x x₁ =
  [ (λ secb≤l →
        let wr = non-interference-bexp b e secb≤l
         in ⊥-elim (true≢false (trans (sym c) (trans wr r))))
  , (λ l<secb →
        let simp = confinement ct z
            simp2 = confinement ct₁ (SecWhile (level-cong (sym (m≤n⇒m⊔n≡n l<secb)) z))
         in trans (trans (sym (simp2 x (missing x₁))) (sym (simp x ((<-transʳ x₁ l<secb))))) (e x x₁))
  ]  (base (sec₆ b) l)
